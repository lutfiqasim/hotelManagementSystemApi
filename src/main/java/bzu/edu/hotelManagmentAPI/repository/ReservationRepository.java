...
public interface ReservationRepository extends JpaRepository<Reservation, Long> {
    ...
    @Query("SELECT r FROM Reservation r WHERE (:id is null or r.id = :id) AND (:name is null or r.userEntity.first_name = :name) AND (:date is null or r.checkinDate = :date)")
    Page<Reservation> findWithIdNameDate(Long id, String name, LocalDate time, Pageable pageable);

    @Query("SELECT r FROM Reservation r WHERE (:id is null or r.id = :id) AND (:name is null or r.userEntity.first_name = :name) AND (:date is null or r.checkinDate = :date)")
    List<Reservation> findWithIdNameDate(Long id, String name, LocalDate time);
